...
app.post("/v1/chat/completions", async (req, res) => {
  try {
    ...
    if (req.body.stream) {
      // --------------------------------------------------------------
      // <<< BEGIN MODIFIED SECTION (stream handling) >>>
      // --------------------------------------------------------------
      res.setHeader("Content-Type", "text/event-stream");
      res.setHeader("Cache-Control", "no-cache");
      res.setHeader("Connection", "keep-alive");
      // 立即送出一個空的起始事件，避免某些瀏覽器在收到第一筆資料前認為連線已關閉
      res.write("data: {\"object\":\"chat.completion.chunk\",\"id\":\"\",\"model\":\"\",\"choices\":[]}\n\n");
      const reader = cfRes.body.getReader();
      const decoder = new TextDecoder();
      let buf = "";
      let streamEnded = false;               // <-- 新增：追蹤是否已收到 [DONE]
      while (true) {
      ...
      // 若上游流結束卻未送出 [DONE]，補發一次以保證客戶端收到結束訊號
      if (!streamEnded) {
        res.write("data: [DONE]\n\n");
      }
      res.end();
      // --------------------------------------------------------------
      // <<< END MODIFIED SECTION (stream handling) >>>
      // --------------------------------------------------------------
    } else {
    ...
    // --------------------------------------------------------------
    // <<< BEGIN MODIFIED SECTION (error handling) >>>
    // --------------------------------------------------------------
    // 若回應已開始（例如已寫入 SSE 標頭），必須在流中回傳錯誤再關閉
    if (!res.headersSent) {
    ...
    // --------------------------------------------------------------
    // <<< END MODIFIED SECTION (error handling) >>>
    // --------------------------------------------------------------
  }
});
